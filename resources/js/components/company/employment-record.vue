        ...
        <table class="table table-bordered table-hover table-striped ">
            ...
            <tbody>
              <tr v-for="(employee,index) in employees" :key="'employee'+index">
                <th >{{index+1}}</th>
                <td>
                 <a :href="'/company/applicant/'+employee.applicant_account_id">
                    {{employee.lname}},
                    {{employee.fname}} 
                    {{employee.mname}}
                 </a>   
                ...
                <td>
                    <end-del v-if="employment_status == 'ongoing'" :employment_id="employee.id" :employee_id="employee.applicant_account_id"></end-del>
                    <applicant-rating v-else :employee='employee' :index='index'></applicant-rating>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
    ...
    methods:{
        ...
        getEmployees(url = '/company/returnEmployees'){
            let vm = this;
            vm.current_page = url;
            axios.post(url,{
                search:vm.search,
